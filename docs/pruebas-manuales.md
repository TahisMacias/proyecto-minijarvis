...
- [ ] **1.1** Doble clic en `Iniciar Mini-JARVIS.bat` -> abre la ventana.
- [ ] **1.2** El circulo de arriba esta **punteado y quieto** (reposo), y abajo dice
      "Listo. Manten presionado el boton para hablar".
- [ ] **1.3** Cierra la ventana con la X -> se cierra entera, sin dejar nada colgado.

## 2. Hablar (las dos formas)

- [ ] **2.1** Manten el **boton** con el raton, habla, suelta -> responde.
- [ ] **2.2** Manten la **barra espaciadora**, habla, suelta -> responde igual.
- [ ] **2.3** Mientras hablas, el circulo esta **verde y latiendo**. Es la senal de
      que si esta grabando.
- [ ] **2.4** Al soltar, pasa a **azul con tres puntos** (pensando) y luego a **rosa
      con ondas** (respondiendo).

## 3. Memoria de la conversacion

- [ ] **3.1** Pregunta algo. Ejemplo: *"Mi color favorito es el verde"*.
- [ ] **3.2** En el turno siguiente pregunta: *"¿Cual dije que era mi color favorito?"*
      -> debe acordarse. Eso prueba que el historial se envia completo.
- [ ] **3.3** Encadena un tercer turno con otra referencia al anterior -> sigue el hilo.

## 4. Que no se congele

- [ ] **4.1** Mientras esta **pensando o respondiendo**, mueve la ventana arrastrandola.
      Debe moverse con normalidad; nunca debe salir "(no responde)".
- [ ] **4.2** Mientras responde, cambia de pestana a **Laboratorio** y vuelve. Responde
      sin trabarse.
- [ ] **4.3** Redimensiona la ventana durante un turno -> se acomoda sin romperse.

## 5. Los errores, que es lo que mas importa

- [ ] **5.1** Presiona y **suelta de inmediato**, sin decir nada -> debe aparecer un
      aviso amable tipo "No te escuche bien" y volver a reposo. **No** debe quedarse
      pensando para siempre.
- [ ] **5.2** Mientras esta respondiendo, presiona la barra otra vez -> te avisa que
      esperes; no abre el microfono encima del turno en curso.
- [ ] **5.3** **Apaga el wifi** y habla -> mensaje pidiendo revisar la conexion, el
      circulo pasa a **durazno con triangulo** y despues vuelve a reposo.
- [ ] **5.4** Vuelve a **encender el wifi** y habla otra vez -> funciona sin reiniciar
      la aplicacion.
- [ ] **5.5** En ningun caso aparece texto tecnico raro (`Traceback`, `Error 500`,
      nombres de archivos `.py`). Si aparece, copialo tal cual.

## 6. Los estados sin leer

- [ ] **6.1** Mira la ventana **de lejos, sin leer el texto**: ¿sabes en cual de los
      cuatro estados esta, solo por el color y la figura?
- [ ] **6.2** Lo mismo entrecerrando los ojos hasta que el color casi no se distinga:
...
- [ ] **7.1** Habla una frase y abre la pestana **Laboratorio**. La primera vez tarda
      unos 20 segundos (esta cargando el modelo); despues es inmediato.
- [ ] **7.2** Muestra **tu frase** cortada en tokens con su numero, y el mapa de calor
      de esa misma frase. No una frase de ejemplo: la tuya.
- [ ] **7.3** Mientras calcula, la conversacion sigue funcionando.

## 8. El laboratorio por consola

- [ ] **8.1** Con el entorno activado: `python -m exploration.transformer_lab`.
      Termina sin errores e imprime tokens, IDs y las formas de los tensores.
- [ ] **8.2** Abre `exploration/mapa_atencion.png` -> se ven los tokens etiquetados en
      los dos ejes y una franja clara pegada a la diagonal.
- [ ] **8.3** ¿Podrias explicar esa imagen en voz alta a alguien? Si no, dime que
      parte no se entiende: es material de sustentacion.

## 9. Alguien mas

- [ ] **9.1** Pidele a otra persona que use la aplicacion **sin explicarle nada** y
